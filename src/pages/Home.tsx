...
const YearProgress = ({ tasks }: { tasks: Task[] }) => {
  ...
  return (
    <section className="mx-4 mt-4 p-4 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
      <div className="flex items-end justify-between mb-2">
        <div>
          <p className="text-xs text-gray-400 dark:text-gray-500 font-medium">연간 목표 달성률</p>
      ...
      <p className="text-[10px] text-gray-300 dark:text-gray-600 mt-1 text-right">
        올해 {dayPct}% 경과
      </p>
    </section>
  );
...
const WeekSection = ({ tasks }: { tasks: Task[] }) => {
  ...
  return (
    <section className="mx-4 mt-4">
      <h2 className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-2 px-1">이번 주</h2>
      {/* 요일 바 */}
      ...
      {/* 이번 주 Task 목록 */}
      {weekTasks.length === 0 ? (
        <p className="text-xs text-gray-300 dark:text-gray-600 text-center py-3">
          이번 주 등록된 목표가 없습니다
        </p>
      ) : (
  ...
  return (
    <section className="mx-4 mt-5 mb-4">
      <div className="flex items-center justify-between mb-2 px-1">
        <h2 className="text-sm font-semibold text-gray-500 dark:text-gray-400">오늘 할 일</h2>
